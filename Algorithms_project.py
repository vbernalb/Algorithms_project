...
MAX_SHORT_POSITION_SIZE = 2.4 / TOTAL_POSITIONS
...
def make_pipeline():
    ...
    revenue = Fundamentals.revenue_growth.latest
    g_score = Fundamentals.growth_score.latest
    quick_r = Fundamentals.quick_ratio.latest
    payment_t = Fundamentals.payment_turnover.latest
    ...
    revenue_winsorized = revenue.winsorize(min_percentile=0.1, max_percentile=0.9)
    g_score_winsorized = g_score.winsorize(min_percentile=0.1, max_percentile=0.9)
    quick_r_winsorized = quick_r.winsorize(min_percentile=0.1, max_percentile=0.9)
    payment_t_winsorized = payment_t.winsorize(min_percentile=0.1, max_percentile=0.9)
    ...
    combined_factor = (
        ...
        margin_winsorized.zscore() +
        revenue_winsorized.zscore() +
        g_score_winsorized.zscore() +
        quick_r_winsorized.zscore() +
        payment_t_winsorized.zscore() 
    )
    ...
    # day.
    longs = combined_factor.top(TOTAL_POSITIONS//2.3, mask=universe)
    shorts = combined_factor.bottom(TOTAL_POSITIONS//1.3, mask=universe)
